...
We do not use geom_line() because it has lines connecting each dot. geom_smooth() will show a smoother line that shows the trend clearly.
...
boxplot = f + geom_boxplot()  x, y, lower, middle, upper, ymax, ymin, alpha,  color, fill, group, linetype, shape, size, weight

histogram = c + geom_histogram(binwidth = 5)  x, y, alpha, color, fill, linetype, size, weight 

area chart = c + geom_area(stat = "bin") x, y, alpha, color, fill, linetype, size 
...
```
The two graphs do not look different because it is telling R to do the same thing. Only the part with the x and y axis was moved to be able to make changes to all parts of the graph.
